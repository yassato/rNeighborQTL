...
test_that(
  ...
  code = {
    ...
    f2_scan1 <- qtl::scanone(genoprobs_f2,pheno.col=fake_f2$pheno[,1],addcovar=as.matrix(cbind(fake_f2$pheno$sex,fake_f2$pheno$pgm)),method="hk")
    bc_scan1 <- qtl::scanone(genoprobs_bc,pheno.col=fake_bc$pheno[,1],addcovar=as.matrix(cbind(fake_bc$pheno$sex,fake_bc$pheno$pgm)),method="hk")

    expect_equal(round(stats::cor(colkas_scan$LOD_self, colkas_scan1[1:nrow(colkas_scan),3]),2),1)
    expect_equal(round(stats::cor(f2_scan$LOD_self, f2_scan1[1:nrow(f2_scan),3]),2),1)
    expect_equal(round(stats::cor(bc_scan$LOD_self, bc_scan1[1:nrow(bc_scan),3]),2),1)
  }
)
...
test_that(
  ...
  code = {
    ...
    expect_equal(colkas_scan2["c4_nga8",4],0)
