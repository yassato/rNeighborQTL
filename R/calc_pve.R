...
#'
#' A function to calculate PVE by neighbor effects for a series of neighbor distance (\code{d_range}) using a linear mixed model.
...
#'
#' s_seq <- quantile(dist(smap_f2),c(0.2*(1:5)))
#' pve_f2_1 <- calc_pve(genoprobs=genoprobs_f2,
...
calc_pve = function(genoprobs, pheno, gmap, contrasts=c(TRUE,TRUE,TRUE), smap, s_seq, addcovar=NULL, grouping=rep(1,nrow(smap)), response="quantitative", fig=TRUE) {
  ...
  res = c()
  for(s in s_seq) { print(paste0("spatial scale at ",round(s,2)))
    ...
    if(response=="quantitative") {
      ...
      print("error: reponse must be 'quantitative' or 'binary'")
    }
  ...
  if(fig==TRUE) {
    PVE = res[,2]
    deltaPVE = PVE - c(0, PVE[1:(length(PVE)-1)]) #delta PVE = PVE_{s+1} - PVE_s
